...
def sample_and_pretokenize_train(total_tokens):
    ...
    for subset_name, subset_tokens in tokenize_tokens.items():
        jsonl_dir = f"./data/combined-datasets/train/{subset_name}"
...
def sample_and_pretokenize_process(kwargs):
    ...
    with open(kwargs["input_path"], "r") as fin, open(kwargs["output_path"], "w") as fout:
        ...
        while True:
            ...
            # Use the custom tokenizer to encode the text
...
def iter_documents_from_sampled_subset(subset_name):
    ...
    files = [f"./data/combined-datasets/train/{subset_name}/{subset_name}_chunk{cid}_tokenized_sampled"
...
def group_documents(target_tokens):
    for subset_name in SUBSET_NAMES:
        subset_dump_dir = f"./data/combined-datasets/{subset_name}"
...
def main():
    ...
    sample_and_pretokenize_train(total_tokens)
    group_documents(total_tokens)

    all_files = []
    for subset_name in SUBSET_NAMES:
        subset_dir = f"./data/combined-datasets/{subset_name}"
